...
from app.assistant_v2.primary.constant import CARD_CONTROLLER_TOOLS
...
class CardController:
    ...
    async def build_agent(  # type: ignore
        ...
        prompt_with_tool = prompt_tmpl.partial(
            to_card_flow_tool_name=ToCardFlow.__name__,
            return_control_tool_name=CompleteOrEscalateTool.__name__,
        )
        agent = prompt_with_tool | prompt.llm_model.bind_tools(CARD_CONTROLLER_TOOLS)  # type: ignore
        return agent
    ...
    async def get_agent(  # type: ignore
        ...
        if not self.ready:
            self.agent = await self.build_agent(
                ctx,
                self.prompt_srv,
                settings.card_controller_prompt,
                settings.card_controller_label,
            )
            self.ready = True
    ...
    async def __call__(
        ...
        if config_data.get(PENDING_RESPONSE_KEY):
            logger.info(
                "There is a response to be sent. Skip LLM. Currently in Card controller."
            )
            return {
        ...
        if (
            ...
            logger.info(
                "User choice is available. Skip LLM. Currently in Card controller"
            )
            tool = ToolCall(
                name=ToCardFlow.__name__,
                id=ctx.request_id(),
                args={},
            )
        ...
        while need_llm:
            ...
            if new_message:
                ...
                logger.info("Valid output received. Currently in Card controller.")
                if (
        ...
        return (
            {
                AssistantStateFields.MESSAGES: [result],
            }
            if result
            else {}
        )
