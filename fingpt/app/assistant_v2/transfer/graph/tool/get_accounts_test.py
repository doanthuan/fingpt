...
from app.assistant_v2.transfer.graph.tool.get_accounts import _get_account
...
async def test_get_account_success(agent_config):
    ...
    with patch(
        "app.assistant_v2.transfer.graph.tool.get_accounts.list_accounts", mock_accounts
    ):
        result = await _get_account(100, agent_config, {})
...
async def test_get_account_failure(agent_config):
    ...
    with patch(
        "app.assistant_v2.transfer.graph.tool.get_accounts.list_accounts",
        side_effect=Exception("API error"),
    ):
        # Call the function and expect an exception
        with pytest.raises(Exception, match="API error"):
            await _get_account(100, agent_config, {})
