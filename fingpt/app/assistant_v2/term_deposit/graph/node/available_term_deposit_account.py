...
from app.assistant_v2.term_deposit.graph.utils import (
    _build_choices_from_term_deposit_accounts,
)
...
available_term_deposit_account_node = NodeName("available_term_deposit_account")


@observe()
async def available_term_deposit_account_func(
    ...
    response = ChatRespDto(
        ...
        metadata=_build_choices_from_term_deposit_accounts(
            state[TermDepositAgentStateFields.TERM_DEPOSIT_ACCOUNTS],
        ),
    )
    config_data[PENDING_RESPONSE_KEY].append(response)
    if any([c.is_enabled for c in response.metadata.choices]):
        return {
            TermDepositAgentStateFields.RESUME_NODE: select_term_deposit_account_node,
        }
    return {
